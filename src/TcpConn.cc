...
TcpConn::TcpConn(int fd, IPoller* poller)
    : fd_(fd), poller_(poller), closed_(false)
{
    SetNonBlocking(fd_);
    io_handler_ = [this](int events){ HandleIO(events); };
    poller_->AddEvent(fd, EPOLLIN | EPOLLRDHUP, &io_handler_);
}
...
{
    poller_->ModEvent(fd_, EPOLLIN | EPOLLRDHUP, &io_handler_);
}

void TcpConn::EnableWrite()
{
    poller_->ModEvent(fd_, EPOLLIN | EPOLLOUT | EPOLLRDHUP, &io_handler_);
}
